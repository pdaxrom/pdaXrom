 ...
 *
 * $Id: dhd_sdio.c,v 1.157.2.27.2.36.4.50 2009/06/05 00:41:56 Exp $
 */
...
static bool sd1idle;
static bool g_chipactive_flag =0;
...
extern struct wake_lock wlan_host_wakelock; //TODO-JJ
#endif
...
int dhdsdio_sched_dpc(dhd_bus_t *bus)
{
	if(bus->dpc_sched != TRUE)
	{
		printk("#### %s DPC Scheduled!!", __FUNCTION__);
		bus->dpc_sched = TRUE;
		dhd_sched_dpc(bus->dhd);
	}
	else
	{
		printk("#### %s DPC already Scheduled!!", __FUNCTION__);
		return -1;
	}
...
int
dhdsdio_bussleep_hack(dhd_bus_t *bus, bool sleep)
{
	...
	if (sleep) {
		...
		dhdcdc_set_ioctl(bus->dhd, 0, WLC_SET_VAR, iovbuf, sizeof(iovbuf));
		printf("### Clear ALL Events \n");		
	} else {
		/* Setup event_msgs */
		bzero(eventmask, sizeof(eventmask));
...
dhdsdio_dpc(dhd_bus_t *bus)
{
	...
	if (dhd_mmc_suspend_state() == TRUE ) {
		printk("%s : enters\n",__FUNCTION__);
		/* 
		* we are already in early_suspend mode. so this isr would probably be pending one in intc of ARM while dhd_suspend is processing. 
		* we simply ignore this pending isr at this moment. otherwise system will be screwed up because sdio bus already started sleeping
		*/
...
#ifdef BCM_HOSTWAKE
	//TODO-JJ is this location appropriate??.. Need to tes ¤©¤¤t more.
	/*Hold a wake lock to avoid suspend-resume to often if there is continuous data
         * transfer. TODO-JJ */
    //printk(KERN_ERR "Holding Wakelock in DPC for 1 sec\n");
	...
	if(!g_chipactive_flag) {
	    ...
	    g_chipactive_flag = 1;
	    printk("[WIFI] Dongle reports CHIP-ACTIVE..\n");
	}
#endif
	...
	if (PKT_AVAILABLE()) {
#if 0 //def BCM_HOSTWAKE
	//TODO-JJ is this location appropriate??.. Need to test more.
	/*Hold a wake lock to avoid suspend-resume to often if there is continuous data
         * transfer. TODO-JJ */
	wake_lock_timeout(&wlan_host_wakelock, 5*HZ);
#endif
