...
NATIVE_SDK_FILES_PREFIX=$(shell echo $$NATIVE_SDK_FILES_PREFIX)
ifeq ("x", "x$(NATIVE_SDK_FILES_PREFIX)")
NATIVE_SDK_FILES_PREFIX="/usr"
NATIVE_SDK_NOT_FOUND="yes"
export NATIVE_SDK_FILES_PREFIX NATIVE_SDK_NOT_FOUND
endif
...
PTXSOURCES_URL		:=  $(shell if [ -f .sources_url ]; then 	\
	                        	cat .sources_url; 		\
				    else 				\
					echo http://mail.pdaXrom.org/src;\
	                	    fi)
...
export TAR TOPDIR BUILDDIR ROOTDIR SRCDIR PTXSRCDIR STATEDIR PACKAGES PTXSOURCES_URL
...
clean: rootclean
	...
	@for i in $$(ls $(TOPDIR)/bootdisk/); do test -d $(TOPDIR)/bootdisk/"$$i"|| rm -f $(TOPDIR)/bootdisk/"$$i"; done
	@for i in $$(ls $(TOPDIR)/bootdisk/feed/); do rm -f $(TOPDIR)/bootdisk/feed/"$$i"; done
	@echo "done."
...
archive-toolchain: virtual-xchain_install
	$(TAR) -C / -jcvf $(TOPDIR)/../$(PTXCONF_GNU_TARGET)-$(GCC_VERSION)-$(BINUTILS_VERSION)-$(GLIBC_VERSION)-$(PTXCONF_FPU_TYPE)-`date +%H.%M_%d.%m.%y`.tar.bz2 \
		$(PTXCONF_PREFIX)
