...
#
#	cp -a $(TOPDIR)/config/pdaXrom/C1000-usb-stuff/root/* $(ROOTFS_DIR)/ipkg/
#
...
#	$(PTXCONF_PREFIX)/bin/mkfs.jffs2 --eraseblock=16k --root=$(ROOTDIR) --little-endian --squash --faketime --devtable=$(TOPDIR)/config/bootdisk/device_table-minimal.txt -n --pad --output=$(TOPDIR)/bootdisk/initrd.jffs2
#else
	$(PTXCONF_PREFIX)/bin/mkfs.jffs2 --eraseblock=16k --root=$(ROOTDIR) --little-endian --squash --faketime --devtable=$(TOPDIR)/config/bootdisk/device_table-minimal-devfs.txt -n --pad --output=$(TOPDIR)/bootdisk/initrd.jffs2
#endif
