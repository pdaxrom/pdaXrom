...
#ifdef BCM_HOSTWAKE
extern int dhdsdio_sched_dpc(struct dhd_bus *);
bool resume_from_isr = FALSE;
...
struct wake_lock wlan_host_wakelock; //TODO-JJ
#endif
...
#if 0 //TODO-JJ
        printk("%s: HOLDING WAKE LOCK FOR 5 SECS \n", __FUNCTION__); 
...
static int dhd_enable_hwakeup(void)
{
	...
	if (ret < 0) {
		printk("Couldn't enable WLAN_HOST_WAKE as wakeup interrupt");
 ...
 */
static void dhd_disable_hwakeup(void)
{
 ...
 */
static irqreturn_t dhd_hostwakeup_isr(int irq, void *dev_id)
{
	int gpio = 0;
	sd_trace((" %s: Enter \n ",__FUNCTION__));
...
int dhd_register_hwakeup(void)
{
	...
	if (ret < 0) {
		printk(KERN_INFO "[WiFi] Failed to get gpio_request \n");
	...
	if (ret < 0) {
		printk(KERN_INFO "[WiFi] Failed to get direction  \n");
		return 0 ;
	}
#endif
	...
	*/
	...
	printk("[WiFi] Host Wake IRQ #: %d \n", dhd_wifi_sleep->host_wake_irq);

	if ( dhd_wifi_sleep->host_wake_irq  < 0 ) {
		printk(KERN_INFO "[WiFi] Failed to get irq  \n");
		return 0 ;
	}
	...
	if (ret) {
		printk(KERN_INFO "[WiFi] Failed to get HostWakeUp IRQ \n");
		free_irq(dhd_wifi_sleep->host_wake_irq, 0);
		return ret;
		/* To do */
	}
	else {
		printk(KERN_INFO"[WiFi] install HostWakeup IRQ \n");
	}

	printk(KERN_INFO "[Wifi] Set IRQ Type to IRQ_TYPE_EDGE_BOTH \n");
