...
dhd_set_mac_address(struct net_device *dev, void *addr)
{
	...
	if (BLOCKABLE()) {
		dhd->set_macaddress = FALSE;
		ret = _dhd_set_mac_address(dhd, ifidx, &dhd->macvalue);
	} else if (dhd->sysioc_pid >= 0) {
		dhd->set_macaddress = TRUE;
		up(&dhd->sysioc_sem);
	} else {
		DHD_ERROR(("%s: non-blockable with no sysioc thread\n", __FUNCTION__));
	}
	return ret;
}
...
int
dhd_sendpkt(dhd_pub_t *dhdp, uint ifidx, void *pktbuf)
{
	int ret;
	dhd_info_t *dhd = (dhd_info_t *)(dhdp->info);
...
dhd_start_xmit(struct sk_buff *skb, struct net_device *net)
{
	...
	DHD_TRACE(("%s: Enter\n", __FUNCTION__));
        ...
        //printk(KERN_ERR "Holding Wakelock in TxPkt for 1 sec\n");
...
dhd_attach(osl_t *osh, struct dhd_bus *bus, uint bus_hdrlen)
{
	...
	/* Allocate etherdev, including space for private structure */
	if (!(net = alloc_etherdev(sizeof(dhd_info_t)))) {
		DHD_ERROR(("%s: OOM - alloc_etherdev\n", __FUNCTION__));
		goto fail;
	}
	dhd = (dhd_info_t *)net->priv;
	memset(dhd, 0, sizeof(dhd_info_t));

	dhd->pub.osh = osh;

	if (dhd_add_if(dhd, 0, (void *)net, net->name) == DHD_BAD_IF)
		goto fail;
...
dhd_net_attach(dhd_pub_t *dhdp, int ifidx)
{
	...
	memcpy(net->dev_addr, temp_ethaddr, ETHER_ADDR_LEN);
