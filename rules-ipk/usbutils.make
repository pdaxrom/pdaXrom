...
###USBUTILS_ENV	+= CFLAGS="-O2 -I$(KERNEL_DIR)/include"
...
$(STATEDIR)/usbutils.install: $(STATEDIR)/usbutils.compile
	@$(call targetinfo, $@)
	asasd
	#rm -rf $(USBUTILS_IPKG_TMP)
	#$(USBUTILS_PATH) $(MAKE) -C $(USBUTILS_DIR) DESTDIR=$(USBUTILS_IPKG_TMP) install
	#cp -a $(USBUTILS_DIR)/usb.h $(CROSS_LIB_DIR)/include/
	#cp -a $(USBUTILS_IPKG_TMP)/usr/lib/* $(CROSS_LIB_DIR)/lib/
	#rm -rf $(USBUTILS_IPKG_TMP)
	touch $@
...
$(STATEDIR)/usbutils.targetinstall: $(usbutils_targetinstall_deps)
	...
	echo "Source: $(USBUTILS_URL)"						>>$(USBUTILS_IPKG_TMP)/CONTROL/control
