...
ifdef PTXCONF_GENEXT2FS
	...
	mkdir -p $(ROOTFS_DIR)/initrd/{bin,dev,etc,lib,mnt,cdrom,sbin,sys,tmp,proc,modules}
	...
	cp -a $(UDEV_IPKG_TMP)/sbin				$(ROOTFS_DIR)/initrd/
	cp -a $(UDEV_IPKG_TMP)/usr/{bin,sbin}			$(ROOTFS_DIR)/initrd/
	mkdir -p $(ROOTFS_DIR)/initrd/etc/udev/
	cp -a $(TOPDIR)/config/pdaXrom-x86/udev/config-initrd/*	$(ROOTFS_DIR)/initrd/etc/udev/
...
ifdef PTXCONF_DEVFSD
	...
	cp -a $(TOPDIR)/config/pdaXrom-x86/linuxrc_cdrom-sysfs $(ROOTFS_DIR)/initrd/linuxrc
...
ifneq ("", $(PTXCONF_VENDORTWEAKS_USERCONFIG))
	if [ -d $(TOPDIR)/config/pdaXrom-x86/cdscripts/$(PTXCONF_VENDORTWEAKS_USERCONFIG) ]; then \
	...
	cp -a $(TOPDIR)/config/pdaXrom-x86/win32tools 	$(ROOTFS_DIR)/cdrom-compressed/
