...
//		printk("[fimc]%s:: cfg->v2.input->index = %d \n", __FUNCTION__, cfg->v2.input->index);
		/* Old input index */
//		old_index = cfg->v2.input->index;
		/* New input index */
		cfg->v2.input = fimc_inputs[index];
//		printk("[fimc]%s:: index = %d, old_index = %d \n", __FUNCTION__, index , old_index);
...
/*
		if (fimc_input_handlers[old_index]->exit_input_handler)
			{
			printk("exit_input_handler is entered !! \n");
...
{
	(printk("[CAM-DRV] +s3c_camif_release\n"));
...
{
	(printk("[CAM-DRV] +s3c_camif_init_sensor\n"));
...
{
	(printk("[CAM-DRV] +s3c_camif_probe\n"));
...
{
	(printk("[CAM-DRV] +s3c_camif_remove\n"));
...
{
	(printk("[CAM-DRV] =s3c_camif_register\n"));
...
{
	(printk("[CAM-DRV] =s3c_camif_unregister\n"));
...
{
	(printk("[CAM-DRV] +s3c_camif_add_sensor\n"));
...
{
	(printk("[CAM-DRV] +s3c_camif_remove_sensor\n"));
