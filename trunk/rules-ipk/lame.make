...
$(STATEDIR)/lame.install: $(STATEDIR)/lame.compile
	...
	cp -a  $(LAME_IPKG_TMP)/usr/include/*        	$(CROSS_LIB_DIR)/include
	cp -a  $(LAME_IPKG_TMP)/usr/lib/*            	$(CROSS_LIB_DIR)/lib
	perl -i -p -e "s,\/usr/lib,$(CROSS_LIB_DIR)/lib,g" $(CROSS_LIB_DIR)/lib/libmp3lame.la
	rm -rf $(LAME_IPKG_TMP)
	touch $@
...
$(STATEDIR)/lame.targetinstall: $(lame_targetinstall_deps)
	...
	echo "Source: $(LAME_URL)" 						>>$(LAME_IPKG_TMP)/CONTROL/control
	...
	echo "Source: $(LAME_URL)" 						>>$(LAME_IPKG_TMP)-gtk/CONTROL/control
