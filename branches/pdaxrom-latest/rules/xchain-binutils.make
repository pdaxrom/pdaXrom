...
XCHAIN_BINUTILS_BUILD_DIR = $(XCHAIN_BUILDDIR)/$(XCHAIN_BINUTILS)-build
# ----------------------------------------------------------------------------
# Get
# ----------------------------------------------------------------------------
...
$(STATEDIR)/xchain-binutils.prepare: $(STATEDIR)/xchain-binutils.extract
	@$(call targetinfo, $@)
	@$(call clean, $(XCHAIN_BINUTILS_BUILD_DIR)/config.cache)
	mkdir -p $(XCHAIN_BINUTILS_BUILD_DIR)
	cd $(XCHAIN_BINUTILS_BUILD_DIR) && $(XCHAIN_BINUTILS_ENV) \
		$(XCHAIN_BINUTILS_DIR)/configure $(XCHAIN_BINUTILS_AUTOCONF)
	touch $@
...
$(STATEDIR)/xchain-binutils.compile: $(STATEDIR)/xchain-binutils.prepare 
	@$(call targetinfo, $@)
	make -C $(XCHAIN_BINUTILS_BUILD_DIR)
	touch $@
...
$(STATEDIR)/xchain-binutils.install: $(STATEDIR)/xchain-binutils.compile
	@$(call targetinfo, $@)
	make install -C $(XCHAIN_BINUTILS_BUILD_DIR)
...
# shared one (.so)
#
	cd $(XCHAIN_BINUTILS_BUILD_DIR)/libiberty && \
		ld --whole-archive libiberty.a -r -o libiberty-$(XCHAIN_BINUTILS_VERSION).so

	install -m 755 -D $(XCHAIN_BINUTILS_BUILD_DIR)/libiberty/libiberty-$(XCHAIN_BINUTILS_VERSION).so \
...
# expects it
#
	install -m 644 -D $(XCHAIN_BINUTILS_BUILD_DIR)/libiberty/libiberty.a \
...
xchain-binutils_clean: 
	rm -rf $(STATEDIR)/xchain-binutils.* $(XCHAIN_BINUTILS_BUILD_DIR)
