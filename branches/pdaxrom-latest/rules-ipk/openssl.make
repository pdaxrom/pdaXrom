...
$(STATEDIR)/openssl.prepare: $(openssl_prepare_deps)
	...
	cd $(OPENSSL_DIR) && \
		$(OPENSSL_PATH) \
		./Configure $(THUD) $(OPENSSL_AUTOCONF) no-krb5
	touch $@
...
#
ifdef PTXCONF_ARCH_MIPS
	$(OPENSSL_PATH) $(MAKE) -C $(OPENSSL_DIR) INSTALLTOP=$(CROSS_LIB_DIR) EX_LIBS="-ldl -lc" openssl.pc
	$(OPENSSL_PATH) $(MAKE) -C $(OPENSSL_DIR) $(OPENSSL_MAKEVARS) EX_LIBS="-ldl -lc"
else
...
ifdef PTXCONF_OPENSSL_SHARED
	...
	echo "Source: $(OPENSSL_URL)"						>>$(OPENSSL_IPKG_TMP)/CONTROL/control
