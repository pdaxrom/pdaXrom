...
ifdef PTXCONF_KERNEL_EXTERNAL_GCC
	...
	cd $(PCMCIA-CS_DIR) && \
		    $(PCMCIA-CS_PATH) $(PCMCIA-CS_ENV) \
		    ./Configure --kernel=$(KERNEL_DIR) --noprompt --nocardbus \
		    --arch=arm --ucc=$(PTXCONF_GNU_TARGET)-gcc \
...
$(STATEDIR)/pcmcia-cs.targetinstall: $(pcmcia-cs_targetinstall_deps)
	...
	$(INSTALL) -m 755 -D $(PCMCIA-CS_DIR)/etc/usb-host      $(PCMCIA-CS_DIR)/ipkg_tmp/etc/pcmcia/usb-host
	$(INSTALL) -m 644 -D $(PCMCIA-CS_DIR)/etc/rex-cfu1.conf $(PCMCIA-CS_DIR)/ipkg_tmp/etc/pcmcia/rex-cfu1.conf
...
ifdef PTXCONF_KERNEL_RMK_PXA_EMBEDIX_SL5500
	...
	chmod +s $(PCMCIA-CS_DIR)/ipkg_tmp/sbin/cardctl
	mkdir -p $(PCMCIA-CS_DIR)/ipkg_tmp/CONTROL
	echo "Package: pcmcia-cs" 						 >$(PCMCIA-CS_DIR)/ipkg_tmp/CONTROL/control
	echo "Source: $(PCMCIA-CS_URL)"						>>$(PCMCIA-CS_DIR)/ipkg_tmp/CONTROL/control
	...
	echo "#!/bin/sh"				 >$(PCMCIA-CS_DIR)/ipkg_tmp/CONTROL/postinst
