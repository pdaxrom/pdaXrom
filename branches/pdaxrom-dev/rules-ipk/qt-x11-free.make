...
QT-X11-FREE_AUTOCONF += -xplatform powerpc-linux-g++
endif
...
ifdef   PTXCONF_ARCH_PPC
	rm -rf  $(QT-X11-FREE_DIR)/mkspecs/powerpc-linux-g++
	cp -a $(TOPDIR)/config/pdaXrom-ppc/$(QT-X11-FREE_VERSION)-powerpc-linux-g++ $(QT-X11-FREE_DIR)/mkspecs
	mv $(QT-X11-FREE_DIR)/mkspecs/$(QT-X11-FREE_VERSION)-powerpc-linux-g++ $(QT-X11-FREE_DIR)/mkspecs/powerpc-linux-g++
	perl -p -i -e "s/\@X11INC@/`echo $(CROSS_LIB_DIR)/include | sed -e '/\//s//\\\\\//g'`/g" $(QT-X11-FREE_DIR)/mkspecs/powerpc-linux-g++/qmake.conf
	perl -p -i -e "s/\@X11LIB@/`echo $(CROSS_LIB_DIR)/lib | sed -e '/\//s//\\\\\//g'`/g" $(QT-X11-FREE_DIR)/mkspecs/powerpc-linux-g++/qmake.conf
	perl -p -i -e "s/\@INCDIR@/`echo $(PTXCONF_PREFIX)/$(PTXCONF_GNU_TARGET)/include | sed -e '/\//s//\\\\\//g'`/g" $(QT-X11-FREE_DIR)/mkspecs/powerpc-linux-g++/qmake.conf
	perl -p -i -e "s/\@LIBDIR@/`echo $(PTXCONF_PREFIX)/$(PTXCONF_GNU_TARGET)/lib | sed -e '/\//s//\\\\\//g'`/g" $(QT-X11-FREE_DIR)/mkspecs/powerpc-linux-g++/qmake.conf
endif
	cd $(QT-X11-FREE_DIR) && \
...
ifdef   PTXCONF_ARCH_PPC
	rm -f $(QT-X11-FREE_DIR)/mkspecs/default
	ln -sf powerpc-linux-g++   $(QT-X11-FREE_DIR)/mkspecs/default
endif
ifndef PTXCONF_QT-X11-FREE-QT4
...
$(STATEDIR)/qt-x11-free.targetinstall: $(qt-x11-free_targetinstall_deps)
	...
	$(INSTALL) -d $(QT-X11-FREE_IPKG_TMP)/usr/lib/qt/plugins/sqldrivers
ifndef PTXCONF_QT-X11-FREE-QT4
	...
	###$(INSTALL) -m 755 $(QT-X11-FREE_DIR)/plugins/sqldrivers/libqsqlite.so $(QT-X11-FREE_IPKG_TMP)/usr/lib/qt/plugins/sqldrivers
	...
	###$(CROSSSTRIP) $(QT-X11-FREE_IPKG_TMP)/usr/lib/qt/plugins/sqldrivers/libqsqlite.so
...
ifndef PTXCONF_QT-X11-FREE-QT4
	...
	echo "Source: $(QT-X11-FREE_URL)"						>>$(QT-X11-FREE_IPKG_TMP)/CONTROL/control
