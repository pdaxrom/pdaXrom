...
$(STATEDIR)/sysvinit.targetinstall: $(sysvinit_targetinstall_deps)
	...
	$(SYSVINIT_PATH) ROOT=$(SYSVINIT_DIR)/ipkg_tmp $(MAKE) -C $(SYSVINIT_DIR) install ROOT=$(SYSVINIT_DIR)/ipkg_tmp INSTALL=install
	rm -rf $(SYSVINIT_DIR)/ipkg_tmp/usr/include
	rm -rf $(SYSVINIT_DIR)/ipkg_tmp/usr/share
	mkdir -p $(SYSVINIT_DIR)/ipkg_tmp/CONTROL
	echo "Package: sysvinit" 						 >$(SYSVINIT_DIR)/ipkg_tmp/CONTROL/control
	echo "Source: $(SYSVINIT_URL)"						>>$(SYSVINIT_DIR)/ipkg_tmp/CONTROL/control
