...
ifneq ("",$(GLIBC_TARGET_CFLAGS))
GLIBC_ENV	+= CFLAGS="$(GLIBC_TARGET_CFLAGS)"
endif

ifdef GLIBC_CONFIGPARMS
GLIBC_ENV	+=  $GLIBC_CONFIGPARMS
endif
...
# HACK!!!! HACK!!!!
#ifdef PTXCONF_OPT_MIPSEL_R5900
#ifdef PTXCONF_GCC_2_95_2
#GLIBC_AUTOCONF =\
#		--build=$(GNU_HOST) \
#		--host=$(PTXCONF_GNU_TARGET) \
#		--target=$(PTXCONF_GNU_TARGET) \
...
ifdef PTXCONF_GLIBC_LIBRT
	...
	mkdir -p $(GLIBC_DIR)/ipkg/etc
	cp -f $(CROSS_LIB_DIR)/etc/rpc $(GLIBC_DIR)/ipkg/etc/

	cp -f $(CROSS_LIB_DIR)/sbin/rpcinfo $(GLIBC_DIR)/ipkg/sbin/
	$(GLIBC_STRIP) $(GLIBC_DIR)/ipkg/sbin/rpcinfo

	cp -f $(CROSS_LIB_DIR)/sbin/ldconfig $(GLIBC_DIR)/ipkg/sbin/
	$(GLIBC_STRIP) $(GLIBC_DIR)/ipkg/sbin/ldconfig

ifndef PTXCONF_LIBICONV
	mkdir -p $(GLIBC_DIR)/ipkg/usr/bin
	cp -f $(CROSS_LIB_DIR)/bin/iconv $(GLIBC_DIR)/ipkg/usr/bin/
	$(GLIBC_STRIP) $(GLIBC_DIR)/ipkg/usr/bin/iconv
	mkdir -p $(GLIBC_DIR)/ipkg/usr/lib/gconv
	cp -f $(CROSS_LIB_DIR)/lib/gconv/gconv-modules $(GLIBC_DIR)/ipkg/usr/lib/gconv/
ifdef PTXCONF_ARCH_X86
	cp -f $(CROSS_LIB_DIR)/lib/gconv/*.so $(GLIBC_DIR)/ipkg/usr/lib/gconv/
	$(CROSSSTRIP) $(GLIBC_DIR)/ipkg/usr/lib/gconv/*.so
else
	PATH=$(CROSS_PATH) 			\
	FEEDDIR=$(FEEDDIR) 			\
	...
	$(TOPDIR)/scripts/bin/make-gconv-ipks.sh $(CROSS_LIB_DIR)/lib/gconv
endif
endif

	mkdir -p $(GLIBC_DIR)/ipkg/CONTROL
	echo "Package: glibc" 							 >$(GLIBC_DIR)/ipkg/CONTROL/control
	echo "Source: $(GLIBC_URL)" 						>>$(GLIBC_DIR)/ipkg/CONTROL/control
...
ifdef PTXCONF_GLIBC_LOCALE
	...
	cp -f $(CROSS_LIB_DIR)/bin/locale $(GLIBC_DIR)/ipkg/usr/bin/
	$(GLIBC_STRIP) $(GLIBC_DIR)/ipkg/usr/bin/locale
	
	mkdir -p $(GLIBC_DIR)/ipkg/CONTROL
	echo "Package: glibc-locale" 						 >$(GLIBC_DIR)/ipkg/CONTROL/control
	echo "Source: $(GLIBC_URL)" 						>>$(GLIBC_DIR)/ipkg/CONTROL/control
	echo "Priority: optional" 						>>$(GLIBC_DIR)/ipkg/CONTROL/control
	echo "Section: Libraries" 						>>$(GLIBC_DIR)/ipkg/CONTROL/control
	echo "Maintainer: Alexander Chukov <sash@pdaXrom.org>" 			>>$(GLIBC_DIR)/ipkg/CONTROL/control
...
ifdef PTXCONF_GLIBC_LOCALEDEF
	...
	cp -f $(CROSS_LIB_DIR)/bin/localedef $(GLIBC_DIR)/ipkg/usr/bin/
	$(GLIBC_STRIP) $(GLIBC_DIR)/ipkg/usr/bin/localedef
	
	mkdir -p $(GLIBC_DIR)/ipkg/CONTROL
	echo "Package: glibc-localedef" 					 >$(GLIBC_DIR)/ipkg/CONTROL/control
	echo "Source: $(GLIBC_URL)" 						>>$(GLIBC_DIR)/ipkg/CONTROL/control
	echo "Priority: optional" 						>>$(GLIBC_DIR)/ipkg/CONTROL/control
	echo "Section: Libraries" 						>>$(GLIBC_DIR)/ipkg/CONTROL/control
	echo "Maintainer: Alexander Chukov <sash@pdaXrom.org>" 			>>$(GLIBC_DIR)/ipkg/CONTROL/control
...
ifdef PTXCONF_GLIBC_LOCALES
	for l in `echo $(PTXCONF_GLIBC_LOCALES_STR)`; do									\
	    rm -rf $(GLIBC_DIR)/ipkg ;												\
	    mkdir -p $(GLIBC_DIR)/ipkg/usr/lib/locale ;										\
	    ll=`echo $$l | sed -e "y/ABCDEFGHIJKLMNOPQRSTUVWXYZ/abcdefghijklmnopqrstuvwxyz/" | sed -e "s/\_/\-/" | sed -e "s/\./\-/"` ;	\
	    lb=`echo $$l | cut -f1 -d'.'` ;											\
	    le=`echo $$l | cut -f2 -d'.'` ;											\
	    echo $$le $$lb $$ll $$l ; 												\
	    I18NPATH=$(CROSS_LIB_DIR)/share/i18n localedef -i $$lb -f $$le $(GLIBC_DIR)/ipkg/usr/lib/locale/$$l ;		\
	    mkdir -p $(GLIBC_DIR)/ipkg/CONTROL ;										\
	    echo "Package: glibc-locale-$$ll" 					 >$(GLIBC_DIR)/ipkg/CONTROL/control ;		\
	    echo "Source: $(GLIBC_URL)" 					>>$(GLIBC_DIR)/ipkg/CONTROL/control ;		\
	    echo "Priority: optional" 						>>$(GLIBC_DIR)/ipkg/CONTROL/control ;		\
	    echo "Section: Libraries" 						>>$(GLIBC_DIR)/ipkg/CONTROL/control ;		\
	    echo "Maintainer: Alexander Chukov <sash@pdaXrom.org>" 		>>$(GLIBC_DIR)/ipkg/CONTROL/control ;		\
	    ...
	    echo "Version: $(GLIBC_VERSION)" 					>>$(GLIBC_DIR)/ipkg/CONTROL/control ;		\
	    echo "Depends: " 							>>$(GLIBC_DIR)/ipkg/CONTROL/control ;		\
	    echo "Description: glibc $$l locale file"				>>$(GLIBC_DIR)/ipkg/CONTROL/control ;		\
	    cd $(FEEDDIR) && $(XMKIPKG) $(GLIBC_DIR)/ipkg	;								\
	done
endif
...
glibc_imageinstall_deps += $(STATEDIR)/libgcc.imageinstall
endif
glibc_imageinstall_deps += $(STATEDIR)/virtual-image.install

$(STATEDIR)/glibc.imageinstall: $(glibc_imageinstall_deps)
	@$(call targetinfo, $@)
	cd $(FEEDDIR) && $(XIPKG) install glibc
ifdef PTXCONF_GLIBC_LOCALE_INSTALL
	cd $(FEEDDIR) && $(XIPKG) install glibc-locale
endif
ifdef PTXCONF_GLIBC_LOCALEDEF_INSTALL
	cd $(FEEDDIR) && $(XIPKG) install glibc-localedef
endif
ifdef PTXCONF_GLIBC_LOCALES
	for l in `echo $(PTXCONF_GLIBC_LOCALES_INSTALL)`; do									\
