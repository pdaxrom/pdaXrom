...
ifdef PTXCONF_ARM_ARCH_PXA
 ...
 ifdef PTXCONF_ARM_ARCH_ARM7500FE
  NEW_CFLAGS=CFLAGS="-O2 -Wall -D_GNU_SOURCE -mcpu=arm7500fe -mtune=arm7500fe"
 else
   ifdef PTXCONF_ARM_ARCH_ARM7
    NEW_CFLAGS=CFLAGS="-O2 -Wall -D_GNU_SOURCE -mcpu=arm7 -mtune=arm7"
   else
    ifdef PTXCONF_ARM_ARCH_LE
      NEW_CFLAGS=CFLAGS="-O2 -Wall -D_GNU_SOURCE -mcpu=strongarm -mtune=strongarm"
    else
...
ifdef PTXCONF_INITSCRIPTS_USLEEP
	...
	echo "Source: $(INITSCRIPTS_URL)"						>>$(INITSCRIPTS_DIR)/ipkg/CONTROL/control
