...
wireless_parse_scanning_event(struct iw_event *event, ap_info *oldinfo)
{
	...
	if (event->cmd==SIOCGIWAP) {
		...
		info->apaddr = strdup(iw_saether_ntop(&event->u.ap_addr, buf));
		info->en_method = NS_WIRELESS_AUTH_OFF;
		info->haskey = FALSE;
		info->key_mgmt = NS_IW_IE_KEY_MGMT_NONE;
		info->group = NS_IW_IE_CIPHER_TKIP;
    ...
    switch (event->cmd) {
        case SIOCGIWESSID: /* ESSID */
			if (!event->u.essid.flags
				...
				info->essid = strndup(event->u.essid.pointer, event->u.essid.length);
			}
            break;
 ...
 * */
int wireless_refresh(int iwsockfd, const char *ifname)
{
	struct iwreq wrq;
